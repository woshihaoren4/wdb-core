...
impl LocalStoreEntity {
    ...
    async fn extension_block(&self,old_sn:u32)->anyhow::Result<()>{
        ...
        let result = self.blocks.async_update(|mut x|async move{
    ...
    async fn append_to_block(&self,sn:u32,key: u64, mut value: &[u8])->WDBResult<u64>{
...
{
    async fn set(&self, key: u64, mut value: &[u8]) -> anyhow::Result<u64> {
        // let value = value.arc();
        loop {
...
mod test{
    ...
    async fn test_local_store_extension_block(){
        let manager = FileBlockManage::new("./database".into(), 1024 * 1024 * 1024, Arc::new(NodeValeCodec)).arc();
        let cache = RWLockCache::default().arc();
        let store = LocalStoreEntity::new(manager,cache).await.expect("LocalStoreEntity.new failed");
        ...
        for i in 0..100_0000{
            store.set(i ,data).await.expect("node local store insert failed");
            if i % 100000 == 0 {
                println!("插入进度 --->{}",i);
            }
        }
    ...
    async fn test_local_store_mut_threads(){
        let manager = FileBlockManage::new("./database".into(), 1024 * 1024 * 32, Arc::new(NodeValeCodec)).arc();
        let cache = RWLockCache::default().arc();
        let store = LocalStoreEntity::new(manager,cache).await.expect("LocalStoreEntity.new failed").arc();

        let data = "男儿何不带吴钩，收取关山五十州，请君暂上凌烟阁，若个书生万户侯".as_bytes();


        let count = AtomicUsize::new(10_0000).arc();
        let start  = std::time::Instant::now();
        for _ in 0..10 {
            let data = data.clone();
