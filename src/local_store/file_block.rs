...
impl FileBlock {
    pub async  fn new(path:String , codec : Arc<dyn Codec>,size:u32)->anyhow::Result<FileBlock>{
        ...
        let offset = AtomicUsize::new(offset as usize);
        //开启缓存模式
        let buffer = FileBlock::start_cache_schedule(file.clone(), 1024 * 1024 * 4, 1024*16).await;

        let lock = Mutex::new(());
        let cache = RwLock::new(cache).arc();
        FileBlock{status,path,size,codec,file, mf, buffer,offset,lock,cache}.ok()
    }
    ...
    pub async fn scan(codec: Arc<dyn Codec>,mf:Arc<NullLock<MemoryFileReadOnly<'static>>>,sender:Sender<WDBResult<(u64, u64, Vec<u8>)>>){
        ...
        let mut i = 0 ;
        while i < size {
            let result = mf.map::<anyhow::Result<u32>,_>(|x| {
                if i + 4 >= size{
            ...
            let val_len = match result {
                ...
                Err(e) => {
                    ...
                    sender.send(WDBError::BlockAbnormal(i as u64).err()).await.unwrap();
                    break
                }
            };
            if val_len > size {
            ...
            let result = mf.map::<anyhow::Result<Vec<u8>>, _>(|x| {
                if i + 4 + val_len > size {
    ...
    pub async fn buffer_append(&self,key:u64, value: &[u8]) -> WDBResult<u64>{
        ...
        self.offset.store(offset+len as usize,Ordering::Relaxed);
    ...
    pub async fn start_cache_schedule(file:Arc<NullLock<File>>,buf_max:u32,chan_max:usize)->Sender<Vec<u8>>{
        ...
        tokio::spawn(async move{
            while !receiver.is_closed() {
                ...
                if w.is_none() {
                    wd_log::log_panic!("cache_schedule file inner is nil");
                }
...
impl Block for FileBlock{
    ...
    async fn get(&self, offset: u64) -> WDBResult<(u64,Vec<u8>)> {
        if self.status.load(Ordering::Relaxed) == 1 { //尝试从缓存拿
        ...
        if self.status.load(Ordering::Relaxed) == 2 { //尝试从内存中取值
            let offset = offset as usize;
            let size = self.size;
            let codec = self.codec.clone();
            let value = self.mf.map::<anyhow::Result<(u64,Vec<u8>)>, _>(|x|{
                ...
                let res = codec.decode(val)?;res.ok()
            }).await;
            return match value {
        ...
        tokio::spawn(async move {
            ...
            if !r.is_empty() {
                for (offset,(key,val)) in r.deref().iter(){
                    sender.send((offset.clone(),key.clone(),val.clone()).ok()).await.unwrap();
                }
                return
            }
